...
def process(page, label=""):
    ...
    for m in page:
        ...
        print(f"  → {comm_count} community | {civ_count}/8 with position | {len(members)} total players")

        if len(members) < 8:
            print(f"  → SKIP (only {len(members)} players — likely a dropped match)")
            skipped += 1
            continue

        # Skip very short games (under 10 minutes) — dropped/restored before real play
        completion = m.get("completiontime")
        start      = m.get("startgametime")
        duration   = (completion - start) if (completion and start) else None
        if duration is not None and duration < 600:
            print(f"  → SKIP (duration {duration}s < 10 min — likely dropped)")
            skipped += 1
            continue

        # Skip duplicate lineups — same community players within 30 min of another match
        match_comm_ids = {str(x["profile_id"]) for x in members if str(x["profile_id"]) in community_ids}
        match_start    = m.get("startgametime") or m.get("completiontime") or 0
        is_duplicate   = False
        for existing in match_map.values():
            ex_start = existing.get("startgametime") or existing.get("completiontime") or 0
            if abs(match_start - ex_start) > 1800:  # more than 30 min apart
                continue
            ex_comm = {str(x["profile_id"]) for x in existing["matchhistorymember"] if str(x["profile_id"]) in community_ids}
            if len(ex_comm) < 4:
                continue
            overlap = len(match_comm_ids & ex_comm)
            if overlap >= len(match_comm_ids) * 0.75:  # 75%+ same players = duplicate
                is_duplicate = True
                break
        if is_duplicate:
            print(f"  → SKIP (duplicate lineup within 30 min of existing match)")
            skipped += 1
            continue

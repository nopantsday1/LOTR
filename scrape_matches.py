...
def decode_slotinfo(b64):
    ...
    try:
        ...
        decompressed = zlib.decompress(raw, -zlib.MAX_WBITS)  # strip zlib header
        ...
        for i, s in enumerate(slots):
            ...
            result.append({
                ...
                'civ_position': COLOR_TO_CIV.get(int(color_id)),  # 'p1'..'p8' or None
            })
        return result
    except Exception as e:
...
try:
    ...
    for lobby in lobbies:
        ...
        for s in slots:
            ...
            print(f"    {name} → colorId={s['color_id']} → {pos}")
            if s['civ_position']:
...
if unknown_pids:
    ...
    for i in range(0, len(batch_list), 20):
        ...
        try:
            url = (
                f"https://aoe-api.worldsedgelink.com/community/leaderboard/"
                f"getLeaderboardProfiles?title=age2"
                f"&profile_ids={json.dumps([int(p) for p in batch])}"
            )

...
def process(page, label=""):
    ...
    for m in page:
        ...
        match_comm_ids = {x["profile_id"] for x in members if x["profile_id"] in community_ids}
        ...
        for existing in match_map.values():
            ...
            if abs(match_start - ex_start) > 1800:  # more than 30 min apart
                continue
            ex_comm = {x["profile_id"] for x in existing["matchhistorymember"] if x.get("is_community")}
            if len(ex_comm) < 4:
                continue

 ...
 * Access-Control-Allow-Origin header. This Worker fetches it server-side and
 * returns the result with CORS headers, so the site's "check for a lobby"
 * buttons can show what is open *right now* instead of re-reading lobby.json,
 * which a scheduled job only regenerates every ~5-15 minutes.
 *
 * Deploying this is optional. With LIVE_LOBBY_PROXY unset in js/core/config.js
 * the site keeps using the committed file exactly as before.
 *
 * Deploy (once):
 *   cd worker
 *   npx wrangler deploy
 * Then put the printed https://<name>.<subdomain>.workers.dev URL into
 * LIVE_LOBBY_PROXY in js/core/config.js.
 *
 * Deliberately NOT a general-purpose proxy: the upstream URL is hardcoded and
 * no caller-supplied URL is honoured, so this cannot be abused to relay
 * arbitrary traffic through your account.
 */
...
function slim(raw) {
  ...
  return {
    matches: slimMatches,
    avatars,
    // Present only on live responses; the client uses it to say "just now"
    // rather than quoting the committed file's age.
    fetchedAt: Date.now(),

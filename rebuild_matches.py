...
def fetch_and_store(pid_list, label):
    ...
    for m in page:
        ...
        match_comm = {str(x["profile_id"]) for x in members if x["profile_id"] in community_ids}
        ...
        for ex in match_map.values():
            ...
            if abs(match_start - ex_start) > 1800:
                continue
            ex_comm = {str(x["profile_id"]) for x in ex["matchhistorymember"] if x.get("is_community")}
            if len(ex_comm) < 4:
                continue

"""
rebuild_matches.py — simplified single-pass version
Fetches community matches via one bulk query, decodes positions, saves matches.json.
"""
...
def process_match(m, match_map, community_ids):
    """Process a single match entry. Returns True if added."""
    ...
    if mid in match_map:
        # Backfill civ positions if we now have slotinfo
    ...
    comm = sum(1 for x in members if str(x["profile_id"]) in community_ids)
    if comm < 4:
        return False

    # Duplicate check — same community players within 30 min
    ...
    for ex in match_map.values():
        ...
        if len(ex_comm) >= 4 and len(match_comm & ex_comm) >= len(match_comm) * 0.75:
            return False
...
while True:
    pids = json.dumps(all_pids, separators=(",", ":"))
    url  = HISTORY_URL.format(pids=urllib.parse.quote(pids, safe="[],:"), start=start)
    try:
        page = fetch_url(url, timeout=20).get("matchHistoryStats", [])
    except Exception as e:
        print(f"  start={start}: fetch failed — {e}")
        break

    if not page:
        print(f"  start={start}: no more matches")
        break

    added = sum(1 for m in page if process_match(m, match_map, community_ids))
    total_added += added
    print(f"  start={start}: {added} new, {len(page)} returned")
    ...
    if added == 0:
        ...
        if consecutive_empty >= 3:
            print("  Stopping — 3 pages with no new community matches")
            break
    else:
        consecutive_empty = 0
...
print(f"\nTotal: {total_added} new matches found")

# ── Name lookup for non-community players ─────────────────────────────────────
...
print(f"  {has_civ} with position data, {len(filtered)-has_civ} without")
...
for mid, m in sorted(filtered.items(), key=lambda x: x[1].get("completiontime", 0), reverse=True)[:10]:
    ...
    print(f"  {date} | {mid} | {comm}/8 comm | {civ}/8 pos | {m.get('description','')!r}")

...
def fetch_and_store(pid_list, label):
    ...
    while True:
        ...
        try:
            ...
            print(f"  {label} start={start}: fetch failed — {e}")
            break

        if not page:
            print(f"  {label} start={start}: no more matches")
            break
        ...
        total_added += added
        print(f"  {label} start={start}: {added} new, {len(page)} returned")

        if len(page) < 20:
            print(f"  {label}: reached end of history")
            break
        if added == 0:
            consecutive_empty += 1
            if consecutive_empty >= 3:
                print(f"  {label}: stopping — 3 pages with no new matches (likely API limit)")
                break
        else:
            consecutive_empty = 0
    ...
    print(f"  {label}: {total_added} total new matches")
